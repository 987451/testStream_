...
def summarize_text(text, num_sentence=3):
    ...
    sorted_word_freq = sorted(words_freq, key=words_freq.get, reverse=True)

    # top 3개의 빈번하게 나타난 단어 추출
    top_words = sorted_word_freq[:num_sentence]
...
def main():
    subheader = """
    <div style="background-color:rgba(125,225,125,0.3); padding:8px;">
    <h3 style="color:white">텍스트 분석 및 감정분석</h1>
    </div>
    """
    title_text='자연어 처리 프로젝트'
    st.title(title_text)
    st.markdown(subheader, unsafe_allow_html=True)

    # 사이트바를 만들어서/ 사이드바 제목 및 메뉴 선택 적용
    ...
    if menu == "텍스트 분석":
        ...
        if len(raw_text) < 3:
            ...
            target_language = st.selectbox("번역할 언어를 선택하세요",["German", "Spanish", "French", "Italian", "Korean"])
